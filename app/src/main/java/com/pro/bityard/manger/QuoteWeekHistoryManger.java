...
public class QuoteWeekHistoryManger extends Observable {


    private static QuoteWeekHistoryManger quoteWeekHistoryManger;

    private String code;
    private int count;


    public static QuoteWeekHistoryManger getInstance() {
        if (quoteWeekHistoryManger == null) {
            synchronized (QuoteWeekHistoryManger.class) {
                if (quoteWeekHistoryManger == null) {
                    quoteWeekHistoryManger = new QuoteWeekHistoryManger();
                }
            }

        }
        return quoteWeekHistoryManger;
        ...
        NetManger.getInstance().getQuoteHistory(count, "/api/tv/tradingView/history", quote_code, "D", (state, response) -> {
            if (state.equals(BUSY)) {
                ...
                if (quoteChartEntity.getS().equals("ok")) {
                    Log.d("print", "quote:88 "+"QuoteHistory week:"+getCount++);
