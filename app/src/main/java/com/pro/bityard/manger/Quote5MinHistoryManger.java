...
public class Quote5MinHistoryManger extends Observable {


    private static Quote5MinHistoryManger quoteHistoryManger;

    private String code;
    private int count;


    public static Quote5MinHistoryManger getInstance() {
        if (quoteHistoryManger == null) {
            synchronized (Quote5MinHistoryManger.class) {
                if (quoteHistoryManger == null) {
                    quoteHistoryManger = new Quote5MinHistoryManger();
                }
            }
    ...
    public void quote(String quote_code, int time) {
        ...
        NetManger.getInstance().getQuoteHistory(NetManger.QUOTE_HISTORY, time, "/api/tv/tradingView/history", quote_code, "5", (state, response) -> {
            if (state.equals(BUSY)) {
                ...
                if (quoteChartEntity.getS().equals("ok")) {
                    Log.d("print", "quote:88 "+"5Min:"+getCount++);
